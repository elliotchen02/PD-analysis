...
def convert_to_wav(input_file):
    ''' 
    Convert input file to a .wav file using subprocess ffmpeg
    '''
...
def audio_feature(waveFile):
    """ 
    Extract the following audio features:
        volume:
        pause:
        pause pecentage:
        volume change:
...
def voice_features_extraction(voice_file):
    """
    Main function to extract voice features. Called in deployModel. 

    Input:
        voice_file: voice file to be analyzed.

    Output: 
        (a list) [pause percentage, volume change, pitch change, average pitch]
    """
    # wave_file = f"{voice_file[:-4]}.wav"

    # convert voice file to .wav if it doesn't already exist
    ...
    volume, pause, pause_percentage, volumn_change = audio_feature(voice_file)
    average_vol, pitch_change, average_pitch = pitch_feature(voice_file)
